...
impl MehPaths {
    pub fn from_config_dir(config_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        let socket_file = runtime_dir.join(format!("meh-server_{}", id));
        let pid_file    = runtime_dir.join(format!("meh-daemon_{}.pid", id));
        ...
        let log_dir   = cache_base.join("meh");
        let cache_dir = cache_base.join("meh");
